...
class MatchManager extends EventEmitter {
  ...
  processMatchUpdates(matches) {
    for (const match of matches) {
      ...
      // Detect score changes during live matches AND after the match has
      // finished. Data providers sometimes record a late goal (e.g. a
      // stoppage-time own goal) in a poll *after* flipping status to FINISHED,
      // so we must keep applying score corrections during the POST_MATCH window.
      if (LIVE_STATUSES.includes(match.status) || COMPLETED_STATUSES.includes(match.status)) {
        ...
        if (currentHome !== cached.homeScore || currentAway !== cached.awayScore) {
          ...
          // If the match is already finished, the win/loss/draw events were
          // emitted on the FINISHED transition using the pre-correction score.
          // Re-emit them only when the correction flips the outcome
          // (e.g. a late goal turning a draw into a win).
          if (COMPLETED_STATUSES.includes(match.status)) {
            const oldOutcome = this.getResultState(cached.homeScore, cached.awayScore);
            const newOutcome = this.getResultState(currentHome, currentAway);
  ...
  handleScoreChange(match, cached, newHome, newAway) {
    ...
    // Always sync the displayed score, regardless of direction. The
    // team_scored/team_conceded events below only fire on an increase, so a
    // downward correction (e.g. a goal disallowed by VAR after the whistle)
    // would otherwise leave the device showing a stale score.
    this.emitForTeams([homeTeamId, awayTeamId], EVENTS.SCORE_CHANGED, {
      match,
  ...
  /**
   * Get today's match for a team (validated against user's timezone)
   */
